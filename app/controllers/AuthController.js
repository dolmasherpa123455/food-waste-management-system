  ...
  User.findOne({
    ...
    .then((user) => {
      if (user) {
        req.session.user = {
          ...user.dataValues,
          donor: user.dataValues.donor && user.dataValues.donor.dataValues ? user.dataValues.donor.dataValues : {},
        };
...
exports.signUpPage = (req, res, next) => {
  res.render('sign_up', { layout: 'user_layout', signUpPage: true, errorMessage: message(req), oldInput: oldInput(req) });
};
      ...
      if (!user) {
        return bcrypt
          ...
          .then(async (hashedPassword) => {
            ...
            donor.userId = user.dataValues.id;
            await donor.save();
          })
          .then((result) => {
            return res.redirect('/login');
